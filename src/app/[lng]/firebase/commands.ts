...
export async function getContent(
  ...
  try {
    const snapshot: DataSnapshot = await get(ref(db, dbPath.validateContent));
    if (snapshot.exists()) {
      ...
      const data = Object.values(content).filter((item) => {
        ...
        if (organizations !== null) {
          if (organizations.length > 0) {
            org = organizations.some(
              (organization) => item.organization.id === organization.id,
            );
          } else {
            org = true;
          }
        ...
        if (Languages.length > 0) {
          lang = Languages.some((language) => item.languageId === language);
        } else {
          lang = true;
        }
        if (operator === Operator.AND && organizations !== null) {
          return org && tag && lang;
        } else if (operator === Operator.AND && organizations === null) {
          return tag && lang;
        } else {
          return org || tag || lang;
        }
  ...
  try {
    ...
    if (snapshot.exists()) {
      ...
      if (allContent.length === 0) {
        ...
        const newContentRef = await push(ref(db, dbPath.validateContent));
...
export async function deleteContent(index: string) {
  try {
    ...
    if (contentKey !== "") {
      const contentRef = ref(db, `${dbPath.validateContent}/${contentKey}`);
...
export const updateUnusedTags = async (): Promise<void> => {
  try {
    ...
    if (snapshot.exists()) {
      ...
      const organizationSnapshot = await get(ref(db, dbPath.organizations));
      if (tagSnapshot.exists() && organizationSnapshot.exists()) {
        ...
        for (const tag of allTagsValues) {
          const isUsed = allContent.some((content) =>
            content.tags.some((contentTag) => contentTag.id === tag.id),
          );
        ...
        for (const organization of allOrganizationsValues) {
          ...
          let organizationKey = "";
          for (const [key, value] of Object.entries(
            organizationSnapshot.val() as Record<string, Organization>,
          )) {
...
export const getPendingContent = async (): Promise<Content[]> => {
  try {
    const snapshot: DataSnapshot = await get(ref(db, dbPath.pendingContent));
    if (snapshot.exists()) {
...
export async function deleteOrganization(id: string) {
  try {
    ...
    if (organizationKey !== "") {
      const contentRef = ref(db, `${dbPath.organizations}/${organizationKey}`);
      await remove(contentRef);
    } else {
...
export async function updateContent(content: Content): Promise<void> {
  try {
    const snapshot = await get(ref(db, `${dbPath.validateContent}`));
    if (snapshot.exists()) {
      const allContent: [string, Content][] = Object.entries(snapshot.val());
      let contentKey = "";
